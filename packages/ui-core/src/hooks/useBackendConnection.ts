...
async function checkHealth(): Promise<boolean> {
  try {
    // Static import — circular dep is fine; checkHealth() is only called
    // after module initialization is complete (deferred binding via ESM).
    const { fetchBackendHealth } = await import('../api/health');
